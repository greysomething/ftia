...
import { adminSavePitch, adminDeletePitch } from '@/app/admin/pitches/actions'
...
export function PitchAdminForm({ pitch, genres, pitchGenreIds = [], attachments = [] }: Props) {
  const router = useRouter()
  const [state, action, pending] = useActionState(adminSavePitch, null)
  const [visibility, setVisibility] = useState<string>(pitch?.visibility || 'draft')
  const [featured, setFeatured] = useState<boolean>(pitch?.featured || false)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await adminDeletePitch(pitch.id)
  ...
  const creatorName =
    pitch?.user_profiles?.display_name ||
    pitch?.user_profiles?.first_name ||
    'Unknown'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="white-bg p-5">
            ...
            {/* Featured toggle */}
            <label className="flex items-center gap-2 text-sm cursor-pointer mb-4 pt-3 border-t border-gray-100">
              <input
                type="checkbox"
                checked={featured}
          ...
          {/* Creator info */}
          <div className="white-bg p-5">
            <h3 className="font-semibold text-gray-700 mb-3">Creator</h3>
          ...
          {/* Delete card */}
          {pitch?.id && (
            <div className="white-bg p-5 border border-red-200">
              <h3 className="font-semibold text-red-600 mb-3">Danger Zone</h3>
              <p className="text-xs text-gray-500 mb-3">
                Permanently delete this pitch and all its attachments, genre links, and favorites.
              </p>
              <button
                type="button"
                onClick={handleDelete}
                disabled={deleting}
                className="w-full px-4 py-2 text-sm font-medium text-white bg-red-600 rounded hover:bg-red-700 disabled:opacity-50"
              >
