...
export function CrewFilters({ categories, resultCount }: CrewFiltersProps) {
  ...
  const updateFilter = useCallback(
    (key: string, value: string) => {
      ...
      params.delete('page')
      startTransition(() => {
        router.push(`/production-role?${params.toString()}`)
      })
    },
    [router, searchParams]
  )
  ...
  const clearAll = useCallback(() => {
    setSearchValue('')
    startTransition(() => {
      router.push('/production-role')
    })
  }, [router])

  const hasFilters = activeRole || searchParams.get('s')
  ...
  return (
    <div className="space-y-3">
      ...
      {hasFilters && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-[#3ea8c8] p-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {activeRole && (
              <FilterPill
                label={categories.find(c => c.slug === activeRole)?.name ?? activeRole}
      ...
      {/* Role Category Pills */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Role Category</h3>
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => {
            const isActive = activeRole === cat.slug
            return (
              <button
                key={cat.id}
                onClick={() => updateFilter('role', isActive ? '' : cat.slug)}
